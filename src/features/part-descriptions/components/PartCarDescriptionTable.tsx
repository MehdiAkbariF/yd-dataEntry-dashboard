...
import { PartCarDescriptionListItem } from '../types';
import Badge from '@/components/ui/Badge';
import { Switch } from '@/components/ui/Switch';
import { Edit, Trash2, Eye, Loader2, FileText } from 'lucide-react';

interface PartCarDescriptionTableProps {
  items: PartCarDescriptionListItem[];
  isLoading: boolean;
...
export default function PartCarDescriptionTable({
  items,
  isLoading,
  onToggleStatus,
  onDelete,
  isTogglingId,
}: PartCarDescriptionTableProps) {
  if (isLoading) {
    return (
      <div className="flex h-64 w-full items-center justify-center rounded-2xl border border-neutral-800 bg-neutral-900/40">
        <div className="flex flex-col items-center gap-2 text-neutral-400">
          ...
          <span className="text-xs">در حال دریافت توضیحات...</span>
        </div>
      </div>
    );
  }
  ...
  if (items.length === 0) {
    return (
      <div className="flex h-64 w-full flex-col items-center justify-center rounded-2xl border border-neutral-800 bg-neutral-900/40 text-neutral-500">
        <p className="text-sm font-medium">هیچ توضیحات تخصصی یافت نشد.</p>
      </div>
    );
  }
      ...
      <table className="w-full text-right text-xs">
        <thead className="border-b border-neutral-800 bg-neutral-950/80 text-neutral-400">
          <tr>
            ...
            <th className="p-4">مدل‌های خودرو مرتبط</th>
            ...
            <tr key={item.id} className="hover:bg-neutral-800/30 transition-all">
              ...
              <td className="p-4 text-center">
                <div className="flex items-center justify-center gap-1.5">
                  <Link
                    href={`/part-descriptions/view/${item.id}?type=car`}
                  ...
                  <Link
                    href={`/part-descriptions/edit/${item.id}?type=car`}
